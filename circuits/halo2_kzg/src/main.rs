...
use snark_verifier::halo2_base::{gates::circuit::BaseCircuitParams, halo2_proofs};
...
mod common {
    ...
    pub fn gen_proof<C: Circuit<Fr>>(
        ...
        let instances = instances.iter().map(Vec::as_slice).collect::<Vec<_>>();
        let proof = {
            let mut transcript =
...
fn main() {
...

    {
        ...
        serde_json::to_writer(&mut writer, &recursion_params.s_g2()).unwrap();

        let mut transcript =
            PoseidonTranscript::<NativeLoader, _>::new::<SECURE_MDS>(snark.proof.as_slice());

        // write proof
        std::fs::write(format!("{dump_path}/proof.bin"), &snark.proof).unwrap();

        // write protocol
        let file = File::create(format!("{dump_path}/protocol.json")).unwrap();
        let mut writer = BufWriter::new(file);
        serde_json::to_writer(&mut writer, &snark.protocol).unwrap();

        // write instances
        let file = File::create(format!("{dump_path}/instances.json")).unwrap();
        let mut writer = BufWriter::new(file);
        serde_json::to_writer(&mut writer, &snark.instances).unwrap();
